...
A low-maintenance static site with serverless contact and analytics endpoints.
...
Deploy this directory to Vercel. Configure only the variables needed by the two serverless endpoints:
...
See `.env.example` for the expected names. If analytics is not configured, `/api/event` quietly does nothing. If contact delivery fails, the form presents a prefilled email fallback instead of losing the request.
